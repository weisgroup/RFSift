...
bool readMYNTConfig(std::string config_file) {
    ...
    {
        // set defualt camera parameters in rosbag
        Config img_intri_info_default {
            {"calib_model", "kannala_brandt"},
            {"left", {
                {"width",  752},
                {"height",  480},
                {"coeffs", Config::array({ -0.026085148102570024, 0.016356411309886076, -0.03155124842564798, 0.016115684866071813, 368.9498328743533, 368.86728767480594, 379.5404933695057, 244.15072065491216 })},
            }},
            {"right", {
                {"width",  752},
                {"height", 480},
                {"coeffs", Config::array({ -0.020310115091152398, -0.003845567615655925, 0.0015484802700763327, -0.0017014119817931727, 367.92643995950186, 367.98524190983125, 388.99252761999077, 242.1001839488553 })},
            }},
        };

        Config img_extri_info_default {
            {"rotation",    Config::array({ 0.999947958164402, -0.0011958937020723692, -0.010131673163734506, 0.0012422565564183406, 0.9999887814317451, 0.004570974655536934, 0.010126093101064694, -0.004583322911040525, 0.9999382257867742 })},
            {"translation", Config::array({ -120.18136165581151, -0.12558776390039778, -0.633184433243246 })},
        };

        ConversionFromDevice(device_info_path_left, device_info_path_right, img_intri_info_default, img_extri_info_default);
    ...
    // for wireless test
    TEST_FEATURE = fsSettings["test_feature"];
    TEST_FEATURE_NUM = fsSettings["test_feature_num"];
    USE_UWB = fsSettings["uwb"];
    USE_EKF = fsSettings["ekf"];
    UWB_DIS_COV = fsSettings["uwb_dis_cov"];
    RSSI_THRESHOLD = fsSettings["rssi_threshold"];
    UWB_OPTIMIZE = fsSettings["uwb_optimize"];
    VISION_OPTIMIZE = fsSettings["vision_optimize"];
    IMU_OPTIMIZE = fsSettings["imu_optimize"];
    MARGIN_OPTIMIZE = fsSettings["margin_optimize"];
    ...
    if (img_intri != "null" &&
        img_extri != "null") {
      printf("use cam info readings\n");
      CAM_NAMES.push_back(device_info_path_left);
    } else {
      printf("use default cam info\n");
